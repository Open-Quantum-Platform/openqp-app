...
const workflows = [
  ...
  },
  {
    ...
    detail: "NACME overlap starter with a second displaced geometry, matching the current OpenQP example path.",
    jobName: "water_mrsf_nacme",
    runtype: "nacme",
    method: "MRSF-TDDFT",
    functional: "BHHLYP",
    basis: "6-31G",
    ...
    inputExtras: [
      "system2=",
      " 8   0.000000000   0.000000000  -0.031061554",
      " 1  -0.543194329   0.543194329  -0.624469223",
      " 1   0.543194329  -0.543194329  -0.624469223"
    ],
    scfMaxit: 30,
    scfExtras: ["maxdiis=5"],
  ...
  },
  {
    ...
    detail: "Solvent-effect starter for OpenQP builds compiled with DDX/ddPCM support.",
    ...
    scfType: "rhf",
    extraSections: [
      "# Requires OpenQP built with OQP_ENABLE_DDX.",
      "[pcm]",
      "enabled=true",
      "backend=ddx",
      "mode=reference_scf",
...
const functionalOptions = [
  ...
  "BOP", "BP86", "B97", "B97-1", "B97-2", "B97-3", "B97-K", "B3LYP", "B3LYPV1R", "B3LYPV3", "B3LYPV5", "B3LYP*", "REVB3LYP",
...
function openQpFunctionalName(value) {
...
function openQpBasisName(value) {
  const raw = String(value || "").trim();
  const compact = raw.toLowerCase().replace(/\s+/g, "");
  const aliases = {
    "6-31g(d)": "6-31G*",
...
function renderInput() {
  ...
  const selectedFunctional = activeFunctional();
  const functional = openQpFunctionalName(selectedFunctional);
  const selectedBasis = dom.basis.value.trim();
  const inputBasis = openQpBasisName(selectedBasis);
  ...
  const lines = [
    ...
    ...(functional && functional !== selectedFunctional ? [`# Functional alias: ${selectedFunctional} is written as ${functional} for OpenQP compatibility.`] : []),
    ...(inputBasis !== selectedBasis ? [`# Basis alias: ${selectedBasis} is written as ${inputBasis} for OpenQP compatibility.`] : []),
    "",
    "[input]",
